...
-- test 데이터베이스 삭제
-- drop database test;
-- test데이터베이스가 존재하면 test데이터베이스 삭제
...
-- board 테이블에 writer 속성을 member테이블의 id와 연결(외래키 설정)
...
-- board테이블에 잘못된 조회수를 올바르게 수정
alter table board modify views int default 0;
-- 조회수 이름을 view2로 수정 후에 다시 views로 수정
alter table board change views views2 int default 0;
alter table board change views2 views int default 0;
-- board테이블에 있는 제약조건을 삭제
alter table board drop constraint boardboardboard_ibfk_1;
-- board테이블에 writer 와 member테이블에 id를 외래키로 연결(제약조건 추가)
alter table board add constraint fk_boardwriter 
	foreign key(writer) references member(id); 
