...
$(function () {
  ...
  $(".box-body-right2").hover(
    function () {
      clearInterval(rollingRight2);
    },
    function () {
      rollingRight2 = rollingLeft(
        rollingObjRigh2.li,
        rollingObjRigh2.ul,
        rollingObjRigh2.time,
        rollingObjRigh2.duration
      );
    }
  );

  $(".box-body-left2 .list-issue").hover(
    function () {
...
$(function () {
  ...
  $(".box-body-left2 .btn-next").click(function (e) {
    e.preventDefault();
    let obj = $(".box-body-left2 .list-press.selected");
    if (obj.next().length == 0 || !obj.next().hasClass("list-press")) return;
...
$(function () {
  $(".box-body-left3 .box-menu .btn-menu").click(function (e) {
    e.preventDefault();
    $(".box-body-left3 .box-menu .item-menu .btn-menu").attr(
      "aria-selected",
      false
    );
  ...
  $(".box-body-left3 .btn-next").click(function (e) {
    e.preventDefault();
    let obj = $(".box-body-left3 .box-menu .item-menu .btn-menu").filter(
      "[aria-selected=true]"
    );
    if (obj.parent().next().length != 0) obj.parent().next().children().click();
  ...
  $(".box-body-left3 .btn-prev").click(function (e) {
    ...
    if (obj.parent().prev().length != 0) obj.parent().prev().children().click();
...
function drawEmptyboxMenu(tmpMenuArr) {
  ...
  $(".group-service-check [type=checkbox]").each(function () {
    let val = $(this).val();
    if (tmpMenuArr.indexOf(val) != -1) $(this).prop("checked", true);
...
function rollingTop(rollingObj) {
  return setInterval(
    moveTop,
    rollingObj.duration,
    rollingObj.liSelector,
    rollingObj.ulSelector,
    rollingObj.animationTime
  );
}
